		...
		return this.create(data || {}, function (err, session) {
			if (err) {
				return cb(err);
                        ...
                        cookies.set("uid", session.uid(), {expires: new Date(session.expires()), path: sessions.opts.path, domain: sessions.opts.domain});

			return cb(null, session, sessions);
		});
	}

	this.get(cookies.get("uid"), function (err, session) {
		if (err) {
